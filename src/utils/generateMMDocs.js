...
async function h() {
  ...
  const twoColTbl = (headers, rows, colWidths) => new Table({
    ...
    rows: [
      new TableRow({ children: headers.map((hd, i) => new TableCell({
        borders: brd,
        shading: { fill: GREEN, type: ShadingType.CLEAR },
        width: { size: colWidths[i], type: WidthType.DXA }, margins: cm,
      ...
      ...rows.map((row, ri) => new TableRow({ children: row.map((cell, ci) => new TableCell({
        ...
        width: { size: colWidths[ci], type: WidthType.DXA }, margins: cm,
  ...
  const sigBlock = (mmName, mmTitle = 'Authorized Signatory, M&M Operations LLC', counterpartyRole = 'Client') => new Table({
    ...
    rows: [
      new TableRow({ children: ['M&M Operations', counterpartyRole].map(p => new TableCell({
        borders: brd, shading: { fill: GREEN, type: ShadingType.CLEAR },
        width: { size: 4680, type: WidthType.DXA }, margins: cm,
...
export async function generateProposal({ client, event, pricingLog, operator, form, today }) {
  ...
  opts.sections[0].children = [
    ...
    H.infoTbl([
      ['Organization',           client],
      ['Event Name & Date',      `${event?.name || ''} — ${event?.event_date || 'TBD'}`],
    ...
    H.para([H.run('M&M builds and operates a three-layer workforce structure. Every layer has defined authority, defined escalation, and defined accountability.')]),
    H.sp(80),
    H.twoColTbl(
    ...
    H.twoColTbl(
      ...
      [
        ...
        ['Labor Reserve Reconciliation', 'Unused reserve returned post-event. 15% retained as risk allocation fee.'],
      ],
      [4680, 4680]
    ),
    H.sp(80),
    H.notice('On the Labor Reserve: The labor reserve is not a fee — it is a protection mechanism. It funds contingency bench activation if volunteer show rates fall below projection. Unused reserve is reconciled within 14 days of the event.'),
    H.sp(160),
...
export async function generateSOW({ client, event, pricingLog, operator, form, today }) {
  ...
  opts.sections[0].children = [
    ...
    H.para([H.run('Issued under Master Services Agreement  |  M&M Operations  |  v1.0', { color: H.GRAY })]),
    H.sp(160),
    ...
    H.twoColTbl(
      ...
      [
        ...
        ['Labor Reserve Reconciliation', 'Unused reserve reconciled within 14 days post-event. 15% of reserve retained as risk allocation fee.'],
      ],
      [3600, 5760]
    ),
    H.sp(80),
